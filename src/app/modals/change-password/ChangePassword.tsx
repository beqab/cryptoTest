...
const ChangePasswordInner: React.FC<IChangePasswordProps> = () => {
    ...
    const [updatePasswordFetching, setUpdatePasswordFetching] = useState<boolean>(false);
    const [updatePasswordStatus, setUpdatePasswordStatus] = useState<boolean>(false);

    const {errors, register, handleSubmit, watch, reset, setError, clearError} = useForm();
    ...
    const onSubmit = (data: any) => {
        ...
        api.changePassword(data.oldPassword, data.newPassword).subscribe((response) => {
                ...
                {updatePasswordFetching ? null : (
                    <>
                        <p style={{color: "#006EDA"}}>
                            {updatePasswordStatus ? "All changes Successfully Saved" : "Something went wrong"}
                        </p>
                        <button className="sell_button" style={{maxWidth: 230}} onClick={() => setShowModal(false)}>
                            Done
                        </button>
                    </>
                )}
            ...
            <form className="password_form" onSubmit={handleSubmit(onSubmit)}>
                <div className="password_form-inputGroup d-flex">
                    <input
                        ...
                        className={`password_input${errors.oldPassword ? " auth_input-error" : ""}`}
                    ...
                    <input
                        ...
                        className={`password_input${errors.newPassword ? " auth_input-error" : ""}`}
                        ref={register({
                    ...
                    <input
                        ...
                        className={`password_input${errors.newPasswordConfirm ? " auth_input-error" : ""}`}
                        ref={register({
                            required: true,
                            validate: {
