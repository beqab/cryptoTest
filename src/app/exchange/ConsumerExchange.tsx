...
export const ProviderExchange: React.FC<IProviderExchangeProps> = ({
  children,
}) => {
  ...
  useEffect(() => {
    const checkInterval = setInterval(() => {
      ...
      setDiffUpdate(
        utilNumbers.clamp(Math.ceil((nextUpdate - now) / 1000), 0, 9)
      );
  ...
  const start = () => {
    setDiffUpdate(
      utilNumbers.clamp(Math.ceil((nextUpdate - Date.now()) / 1000), 0, 9)
    );
    setIsActive(true);
  };
  ...
  return (
    <ContextExchange.Provider value={context}>
      {children}
    </ContextExchange.Provider>
  );
};
